...
export default function InvoiceBuilder() {
  ...
  // NEW: Advanced E-commerce & Tax State
  const [showAddresses, setShowAddresses] = useState(false);
  const [billingAddress, setBillingAddress] = useState("");
  const [shippingAddress, setShippingAddress] = useState("");
  
  const [showTax, setShowTax] = useState(false);
  const [taxRate, setTaxRate] = useState<number | string>(18); // Default 18% GST
  ...
  const handleGenerate = async () => {
    if (!senderName || !upiId || !clientEmail || totalAmount <= 0) {
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-8 grid lg:grid-cols-2 gap-12 items-start">
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-stone-200">
          <h2 className="font-bold text-[#111827] mb-4">Your Details</h2>
      ...
      <div className="sticky top-24 bg-white shadow-2xl shadow-black/5 rounded-sm p-8 sm:p-12 border border-stone-100 hidden lg:block">
        ...
        {/* NEW: InstaBill Logo embedded in the preview */}
        ...
        <div className="flex justify-between items-start mb-10">
          ...
          <div className="text-right">
            ...
            <p className="font-bold text-[#111827]">{new Date().toLocaleDateString()}</p>
          </div>
        </div>
        ...
        {/* NEW: Subtotal, Tax, and Final Total Logic */}
