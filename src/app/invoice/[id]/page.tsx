...
import { useEffect, useState, use } from "react";
import Image from "next/image"; // Custom Logo Import
import { supabase } from "@/lib/supabase";
import { QRCodeSVG } from "qrcode.react";
        ...
        <div className="lg:col-span-2 flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-12 duration-700 delay-150 print:hidden">
          ...
          {!isPaid && (
            <div className="bg-white p-8 rounded-3xl shadow-xl shadow-black/5 border border-stone-200 flex flex-col items-center text-center relative overflow-hidden">
              ...
              {/* NEW: MOBILE ONLY - Deep Link Button */}
              {!isPendingVerification && (
              ...
              {/* QR CODE - Scaled back slightly on mobile to prioritize the button above */}
              <div className="bg-stone-50 p-5 rounded-3xl border-2 border-stone-100 shadow-inner mb-6 relative w-full flex items-center justify-center">
                <div className={`bg-white p-3 rounded-2xl shadow-sm transition-all duration-500 ${isPendingVerification ? "blur-md opacity-40 grayscale pointer-events-none" : ""}`}>
                  <QRCodeSVG value={upiString} size={200} level="H" includeMargin={false as any} fgColor="#111827" />
                </div>
                {isPendingVerification && <div className="absolute inset-0 flex items-center justify-center"><div className="bg-amber-500 text-white p-4 rounded-full shadow-xl"><Loader2 className="animate-spin" size={36} strokeWidth={3} /></div></div>}
