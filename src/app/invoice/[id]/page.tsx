...
export default function PublicInvoicePage({
  ...
  const hasTax =
    (invoice.tax_rate && invoice.tax_rate > 0) || actualTaxAmount > 0;
  const displayTaxRate =
    invoice.tax_rate ||
    (subtotal > 0 ? Math.round((actualTaxAmount / subtotal) * 100) : 0);
  ...
  return (
    <div className="min-h-screen bg-[#FEF9F2] text-[#111827] flex flex-col items-center py-6 sm:py-12 px-4 sm:px-8 print:bg-white print:py-0 print:px-0">
      ...
      <div className="w-full max-w-5xl mb-6 sm:mb-8 flex items-center gap-2 animate-in fade-in slide-in-from-top-4 duration-700 print:hidden">
        ...
        />
        <span className="text-xl font-bold tracking-tighter text-[#111827]">
          InstaBill
        </span>
      </div>
      ...
      <div className="max-w-5xl w-full grid lg:grid-cols-5 gap-6 lg:gap-10 items-start print:block">
        ...
        <div className="lg:col-span-3 bg-white w-full shadow-2xl shadow-black/5 rounded-2xl sm:rounded-sm p-6 sm:p-12 flex flex-col relative border border-stone-100 animate-in fade-in slide-in-from-bottom-8 duration-700 print:shadow-none print:border-none print:p-0 print:w-full print:max-w-full">
          ...
          <div className="flex justify-between items-start mb-8 sm:mb-12">
            <div>
              <h1 className="text-2xl sm:text-3xl font-light text-slate-300 tracking-wider">
                INVOICE
              </h1>
              <p className="text-xs sm:text-sm font-mono text-slate-400 mt-2 uppercase font-medium">
                #{invoice.id.split("-")[0]}
              </p>
            </div>
            <div className="text-right">
              <p className="text-[10px] sm:text-[11.5px] font-bold text-slate-400 mb-1 uppercase tracking-wider">
                Date
              </p>
              <p className="font-bold text-[#111827] text-lg sm:text-xl tracking-tight">
          ...
          <div className="grid grid-cols-2 gap-6 sm:gap-10 mb-8">
            <div>
              <p className="text-[10px] sm:text-[11.5px] font-bold text-slate-400 mb-1.5 uppercase tracking-wider">
                From
              </p>
              <p className="font-bold text-[#111827] text-lg sm:text-xl tracking-tight">
                {invoice.sender_name}
            ...
            <div className="text-right">
              ...
              <p className="text-base sm:text-xl font-medium text-slate-800 tracking-tight break-all sm:break-normal">
          ...
          <div className="flex-grow">
            <div className="grid grid-cols-5 border-b-2 border-stone-100 pb-3 mb-4">
              <p className="col-span-3 sm:col-span-4 font-bold text-slate-400 text-[10px] sm:text-xs tracking-wider uppercase">
                Description
              </p>
              <p className="col-span-2 sm:col-span-1 text-right font-bold text-slate-400 text-[10px] sm:text-xs tracking-wider uppercase">
                Amount
              </p>
            </div>
            <div className="space-y-4 mb-8">
              {invoice.items &&
                invoice.items.map((item: any, i: number) => (
                  <div
                    key={i}
                    className="grid grid-cols-5 items-start sm:items-center"
                  >
                    <p className="col-span-3 sm:col-span-4 text-sm sm:text-base text-slate-700 font-medium leading-relaxed pr-2">
                      {item.description}
                    </p>
                    <p className="col-span-2 sm:col-span-1 text-right text-[#111827] font-semibold text-base sm:text-lg tracking-tight pt-0.5 sm:pt-0">
                      ₹{item.amount}
                    </p>
                  </div>
                ))}
          ...
          <div className="border-t border-stone-100 pt-6 sm:pt-7 mt-6 sm:mt-8">
            {hasTax && (
              <>
                ...
                <div className="flex justify-between items-center mb-5 sm:mb-6 pb-5 sm:pb-6 border-b border-stone-100">
                  ...
                  <p className="text-base sm:text-lg font-semibold text-slate-700">
                    ₹
                    {actualTaxAmount.toLocaleString("en-IN", {
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between items-start sm:items-end gap-4 sm:gap-0">
              <div>
                <p className="text-[10px] sm:text-[11.5px] font-bold text-slate-400 tracking-wider uppercase mb-1 sm:mb-1.5">
                  Total Amount Due
                </p>
                {isPaid && (
                  <p className="text-xs sm:text-sm font-bold text-green-600 tracking-wider uppercase mt-1 sm:mt-2 border-2 border-green-600 rounded px-2 py-1 inline-block opacity-70 transform -rotate-2">
                    Settled
                  </p>
                )}
                {isPaid && invoice.utr_number && (
                  <div className="mt-3 sm:mt-4 print:mt-8 text-[10px] sm:text-xs text-slate-400 font-mono">
        ...
        <div className="lg:col-span-2 flex flex-col gap-5 sm:gap-6 animate-in fade-in slide-in-from-bottom-12 duration-700 delay-150 print:hidden">
          {!isPaid && (
            <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-xl shadow-black/5 border border-stone-200 flex flex-col items-center text-center relative overflow-hidden">
              <div
                className={`p-3 rounded-2xl mb-4 transition-colors ${isPendingVerification ? "bg-amber-50 text-amber-600" : "bg-indigo-50 text-indigo-600"}`}
              >
                <ShieldCheck size={28} />
              </div>
              ...
              <p className="text-slate-500 text-xs sm:text-sm mb-6 font-medium px-2">
                {isPendingVerification
                  ? "Awaiting manual payout check by freelancer."
              ...
              <div className="bg-stone-50 p-4 sm:p-5 rounded-3xl border-2 border-stone-100 shadow-inner mb-4 relative w-full flex items-center justify-center">
                <div
                  className={`bg-white p-2 sm:p-3 rounded-2xl shadow-sm transition-all duration-500 ${isPendingVerification ? "blur-md opacity-40 grayscale pointer-events-none" : ""}`}
                >
                  <QRCodeSVG
                    value={upiString}
                    size={180}
                    level="H"
                    includeMargin={false as any}
                    fgColor="#111827"
                  />
                </div>
                {isPendingVerification && (
              ...
              <div className="w-full pt-5 border-t border-stone-100 mt-2">
                <div className="flex items-center justify-center gap-1.5 text-slate-500 mb-1">
                  ...
                  <span className="text-[11px] font-bold uppercase tracking-wider">
                    Secured via UPI Network
                  </span>
                </div>
                <p className="text-[10px] text-slate-400 font-medium">
                  100% direct bank-to-bank transfer.
                </p>
              </div>
            </div>
          )}
            ...
            <div className="bg-white p-5 sm:p-6 rounded-3xl shadow-lg shadow-black/5 border border-stone-200 flex flex-col gap-4">
              <div>
                ...
                <p className="text-[10px] sm:text-[11px] text-slate-400 mt-2">
                  Find this 12-digit number (UTR) in your bank/UPI app history.
                </p>
              </div>
              ...
              <button
                ...
                className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 text-white p-3.5 sm:p-4 rounded-xl flex items-center justify-center text-sm sm:text-base font-bold shadow-md transition-all active:scale-[0.98]"
              >
          ...
          {isPendingVerification && (
            <div className="bg-amber-600 p-5 sm:p-6 rounded-3xl flex items-center justify-between shadow-lg shadow-amber-200 text-white">
              <div>
                <p className="text-[10px] sm:text-sm font-bold text-amber-200 tracking-wider uppercase mb-1">
                  Verification Status
                </p>
          ...
          {isPaid && (
            <div className="flex flex-col gap-4 animate-in fade-in zoom-in-95 duration-500">
              <div className="bg-green-600 p-5 sm:p-6 rounded-3xl flex items-center justify-between shadow-lg shadow-green-200 text-white">
                <div>
                  <p className="text-[10px] sm:text-sm font-bold text-green-200 tracking-wider uppercase mb-1">
                    Invoice Status
                  </p>
                  <p className="text-base sm:text-lg font-semibold">
                    Succeeded & Closed
              ...
              <button
                ...
                className="w-full bg-white text-[#111827] hover:bg-stone-50 border border-stone-200 p-4 sm:p-5 rounded-2xl flex items-center justify-center gap-3 text-sm sm:text-base font-bold shadow-sm transition-all active:scale-[0.98]"
              >
